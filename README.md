...
The documentation is designed for use on Linux operating systems.
### Linux
...
- Run the following command by replacing /mnt/ltfs with your tape's mount point:
  ```
...
- You can load multiple volumes simultaneously and tell BRU to process the next volume after the current one is completed using the device cycling command, `-f device`. For example, the following command can be used to read a two-volume tape
  ```
  sudo bru -g -b 2048k -f /dev/nst# -f /dev/nst# -v
  ```
- **TODO** BRU will still ask user input to continue or quit the operation after finishing the first volume, but it will not prompt the user with the *load volume* message. Instead, the user is prompted with two messages: `assuming end of volume #` `use device "/dev/nst#" ? [y/n]` where volume # is the current volume and /dev/nst# is the the next volume loaded. Though BRU recognises the multiple volumes loaded, a way to bypass the user input is yet to be figured out.
...
- Run the following command to restore files with their original path
  ```
...
- You can use either of the following commands, by replacing /tmp/filelist with the path to yoyr filelist:
  ...
  ```
- The above commnads have not been thoroughly tested to ensure if these would restore only the specified files. From the limited testing done, it seems that BRU reads through the entire files upto the specified file instead of seeking the particular file offset. Other commands for selective restoration can be found in the chapter on **File Inclusion and Exclusion** (chapter 5, page 43) and the **Command Line Options: File Selection** section (page 57) in the BRU User's Guide (https://lab.demog.berkeley.edu/Docs/Refs/bru_17_manual.pdf) 
